...
int main(int argc, char *argv[])
{
	...
	int shmid = shmget(IPC_PRIVATE,sizeof(SharedBuffer) * 2, 0666|IPC_CREAT);
	if(shmid == -1){
		perror("error");
		exit(EXIT_FAILURE);
	}

	SharedBuffer (*shb)[2] = shmat(shmid,NULL,0);
	if(shb == (void*)-1){
		exit(-1);	
	}
	printf("shmpid = %d\n",shmid);
...
//		display the expression and result
{
	...
	while(1){
		printf("Input an integer binary expressions(type \"quit\" to finish.):");
		expression[0] = '\0';
		...
		shm_buffer[C2P].is_loaded = FALSE;
		printf("[parent]\"%s\" = %s\n",shm_buffer[P2C].buffer,shm_buffer[C2P].buffer);
...
// 		send the result to the parent process by message passing
{
	...
	while(1){
		...
		strcpy(shm_buffer[P2C].buffer,expression);		
		shm_buffer[P2C].is_loaded = FALSE;	
		strncpy(expression,shm_buffer[P2C].buffer,strlen(shm_buffer[P2C].buffer));
		if(strcmp(expression,"quit") == 0){
			break;
		}
		int res = Evaluate(shm_buffer[C2P].buffer);
		printf("[child]received expression %s\n",shm_buffer[C2P].buffer);
